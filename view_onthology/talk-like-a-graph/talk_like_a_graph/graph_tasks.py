...
class ConnectedNodes(GraphTask):
    ...
    def prepare_examples_dict(
        ...
        for ind, graph in enumerate(graphs):
            ...
            if outgoing_edges:
                answer = (
                    self.get_connected_nodes(outgoing_edges, name_dict) + "."
                )
            else:
                answer = " No nodes."
            examples_dict[ind] = {
    ...
    def get_connected_nodes(
        ...
        if connected_nodes:
            try:
                ...
                connected_nodes_string = ", ".join(
                    map(str, sorted(connected_nodes))
                )
...
class DisconnectedNodes(GraphTask):
    ...
    def create_few_shot_example(
        ...
        if outgoing_edges:
            ...
            if cot:
                answer += (
                    " This is because there is not an edge from %s to %s."
                    % (
                        name_dict[source_node],
                        answer,
                    )
...
class Reachability(GraphTask):
    ...
    def prepare_examples_dict(
        ...
        for ind, graph in enumerate(graphs):
            ...
            question = graph_text_encoders.encode_graph(graph, encoding_method)
            task_description = (
                "Q: Is there a path from node %s to node %s?\nA: "
                % (
                    name_dict[source],
                    name_dict[target],
                )
            ...
            if cot:
                ...
                for i in range(len(path) - 1):
                    ...
                    # The last edge in a path with more than one edge.
                        ...
                        sep = ", and"
                    explanation += (
                        "%s there is an edge from node %d to node %d"
                        % (
                            sep,
                            path[i],
                            path[i + 1],
